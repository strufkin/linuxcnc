...
def wait_for_pin_value(pin_name, value, timeout=5.0):
    ...
    while (time.time() - start_time) < timeout:
        time.sleep(0.1)
        if h[pin_name] == value:
            print("pin '%s' reached target value '%s' after %f seconds" % (pin_name, value, time.time() - start_time), flush=True)
            return
    print("Error: pin '%s' didn't reach value '%s' within timeout of %f seconds (it's %s instead)" % (pin_name, value, timeout, h[pin_name]), flush=True)
    sys.exit(1)
...
print("expecting tool number %d" % expected_startup_tool_number, flush=True)
