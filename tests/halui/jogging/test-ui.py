...
def log(msg):
    ...
    print("%.3f: %s" % (delta_t, msg), flush=True)
    sys.stdout.flush()
...
print("%s starting at %.3f" % (name0, start_position0), flush=True)
print("%s starting at %.3f" % (name1, start_position1), flush=True)
print("%s starting at %.3f" % (name2, start_position2), flush=True)
...
print("jogging %s positive using the private per-joint jog pins" % name0, flush=True)
h[name0 + '-jog-plus'] = 1
...
if h[name0 + '-velocity'] <= 0:
    print("%s did not start jogging" % name0, flush=True)
    sys.exit(1)
...
print("jogging selected joint (%s) negative" % (name1), flush=True)
select_joint(name1)
h['jog-selected-minus'] = 1
...
if h[name1 + '-velocity'] >= 0:
    print("%s did not start jogging" % name1, flush=True)
    sys.exit(1)

if h[name0 + '-velocity'] <= 0:
    print("%s stopped jogging" % name0, flush=True)
    sys.exit(1)

if h[name1 + '-position'] >= start_position1:
    print("%s was selected but did not jog negative (start=%.3f, end=%.3f)" % (name1, start_position1, h[name1 + '-position']), flush=True)
    sys.exit(1)

if h[name2 + '-position'] != start_position2:
    print("%s was not selected but moved (start=%.3f, end=%.3f)" % (name2, start_position2, h[name2 + '-position']), flush=True)
    sys.exit(1)

print("%s was selected and jogged, %s was not selected and stayed still" % (name1, name2), flush=True)
...
print("selecting %s" % name2, flush=True)
select_joint(name2)
...
if h[name1 + '-velocity'] != 0:
    print("%s was deselected but did not stop (start_vel=%.3f, end_vel=%.3f)" % (name1, start_velocity1, h[name1 + '-velocity']), flush=True)
    sys.exit(1)

if h[name2 + '-velocity'] >= 0:
    print("%s was selected but did not move (start_vel=%.3f, end_vel=%.3f)" % (name2, start_velocity2, h[name2 + '-velocity']), flush=True)
    sys.exit(1)

print("%s was deselected and stopped, %s was selected and jogged" % (name1, name2), flush=True)


start_velocity1 = h[name1 + '-velocity']
start_velocity2 = h[name2 + '-velocity']

print("stopping jog", flush=True)
h['jog-selected-minus'] = 0
...
if h[name1 + '-velocity'] != 0:
    print("%s started moving again (start_vel=%.3f, end_vel=%.3f)" % (name1, start_velocity1, h[name1 + '-velocity']), flush=True)
    sys.exit(1)

if h[name2 + '-velocity'] != 0:
    print("%s did not stop (start_vel=%.3f, end_vel=%.3f)" % (name2, start_velocity2, h[name2 + '-velocity']), flush=True)
    sys.exit(1)

print("%s stopped" % name2, flush=True)
