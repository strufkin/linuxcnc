...
def use_pango_font(font, start, count, will_call_prepost=False):
    ...
    #a = bytearray( itertools.repeat(0, 256*256))
...
class GlNavBase:
    ...
    def reset(self):
        ...
        print(dir(self))
        print("glnav reset glGetError() pre",glGetError())
        glMatrixMode(GL_MODELVIEW)
        #print("glnav reset glGetError() post",glGetError())
        glLoadIdentity()
        self._redraw()
    ...
    def set_view_p(self):
        self.reset()
        print("matrix mode",glGetIntegerv(GL_MATRIX_MODE))
        self.perspective = True
        mid, size = self.extents_info()
        glTranslatef(-mid[0], -mid[1], -mid[2])
