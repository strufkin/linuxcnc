...
class Hal_Dial(Gtk.DrawingArea, _HalJogWheelBase):
    ...
    def __init__(self):
        ...
        self.center_color = parse_color('#bdefbdefbdef') # gray
        # private
    ...
    def expose(self, widget, event):
        ...
        event.rectangle(0,0, w.get_width(), w.get_height())
        event.clip()
        #context.rectangle(event.area.x, event.area.y, w.get_width(), w.get_height())
        #context.clip()
        self.draw(event)
        return False
    ...
    def redraw_canvas(self):
        #print(self)
        #print(dir(self))
        alloc = self.get_allocated_size().allocation
        #print(dir(self.get_allocated_size()))       
...
def count_changed(widget, count,scale,delta_scale):
    print ('delta scale = {}'.format(delta_scale))
...
def main():
    ...
    wheel.set_property('center_color', parse_color('#bdefbdefbdef'))
    window.add(wheel)
